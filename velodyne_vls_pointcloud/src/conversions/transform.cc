...
#include "velodyne_vls_pointcloud/transform.h"
...
: tf_prefix_(tf::getPrefixParam(private_nh)), data_(new velodyne_rawdata::RawData())
{
  // Read calibration.
  ...
  srv_ = boost::make_shared<dynamic_reconfigure::Server<velodyne_pointcloud::TransformNodeConfig> >(
    private_nh);
...
void Transform::processScan(const velodyne_msgs::VelodyneScan::ConstPtr & scanMsg)
{
  ...
  for (size_t next = 0; next < scanMsg->packets.size(); ++next) {
    ...
    try {
      ...
      pcl_ros::transformPointCloud(config_.frame_id, *(inPc_.pc), tfPc_, listener_);
