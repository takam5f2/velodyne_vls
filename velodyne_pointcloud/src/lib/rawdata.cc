  ...
  velodyne_pointcloud::VelodyneImplementType impl_type)
{
  ...
  for (int i = 0; i < BLOCKS_PER_PACKET; i++) {
    ...
    {
      // unpack_vls128(pkt, data);
      unpack_vls128(pkt, data, impl_type);
      return;
    }
...
void RawData::unpack_vlp16(const velodyne_msgs::VelodynePacket & pkt, DataContainerBase & data)
{
    ...
    velodyne_pointcloud::VelodyneImplementType impl_type)
  {
    switch (impl_type) {
      case velodyne_pointcloud::VelodyneImplementType::HALF_PRECISION:
        unpack_vls128_half_precision(pkt, data);
        break;
      case velodyne_pointcloud::VelodyneImplementType::FIXED_POINT_16_16:
        unpack_vls128_fixed_16_16_precision(pkt, data);
        break;
      case velodyne_pointcloud::VelodyneImplementType::SINGLE_PRECISION:
      default:
        unpack_vls128_single_precision(pkt, data);
        break;
    }
  }

  /** @brief convert raw VLS128 packet to point cloud with single precision
   *
   *  @param pkt raw packet to unpack
   *  @param pc shared pointer to point cloud (points are appended)
   */
  void RawData::unpack_vls128_single_precision(const velodyne_msgs::VelodynePacket &pkt, DataContainerBase &data) {
    float last_azimuth_diff = 0;
    float azimuth_diff, azimuth_corrected_f;
    uint16_t azimuth, azimuth_next, azimuth_corrected;
    float x_coord, y_coord, z_coord;
  ...
  /** @brief convert raw VLS128 packet to point cloud with half precision
   *
   *  @param pkt raw packet to unpack
   *  @param pc shared pointer to point cloud (points are appended)
   */
  void RawData::unpack_vls128_half_precision(const velodyne_msgs::VelodynePacket &pkt, DataContainerBase &data) {
    using half_float::half;
    using namespace half_float::literal;

    half last_azimuth_diff = 0.0_h;
    half azimuth_diff, azimuth_corrected_f;
    uint16_t azimuth, azimuth_next, azimuth_corrected;
    half x_coord, y_coord, z_coord;
    half distance, intensity;
    const raw_packet_t *raw = (const raw_packet_t *) &pkt.data[0];
    union two_bytes tmp;

    // float time_diff_start_to_this_packet = (pkt.stamp - scan_start_time).toSec();

    half cos_vert_angle, sin_vert_angle, cos_rot_correction, sin_rot_correction;
    half cos_rot_angle, sin_rot_angle;
    half xy_distance;
    ...
    for (int block = 0; block < BLOCKS_PER_PACKET - (4* dual_return); block++) {
      ...
      if (block < (BLOCKS_PER_PACKET - (1+dual_return))) {
        ...
        // Finds the difference between two sucessive blocks
        azimuth_diff = half_float::half_cast<half>((36000 + azimuth_next - azimuth) % 36000);
        ...
        // same as the last to the second to last.
        // Assumes RPM doesn't change much between blocks
        azimuth_diff = (block == BLOCKS_PER_PACKET - (4*dual_return)-1) ? 0.0_h : last_azimuth_diff;
      }
      ...
      if ((config_.min_angle < config_.max_angle && azimuth >= config_.min_angle && azimuth <= config_.max_angle) || (config_.min_angle > config_.max_angle)) {
        for (int j = 0, k = 0; j < SCANS_PER_BLOCK; j++, k += RAW_SCAN_SIZE) {
          ...
          // if (data.pointInRange(distance)) {
          {
            ...
            distance = half_float::half_cast<half>(tmp.uint) * half_float::half_cast<half>(VLP128_DISTANCE_RESOLUTION);
            bool is_invalid_distance = false;
            if(distance == 0.0_h) {
                is_invalid_distance = true;
                distance = 0.3_h;
              ...
              distance += half_float::half_cast<half>(corrections.dist_correction);
            }

            // correct for the laser rotation as a function of timing during the firings
            azimuth_corrected_f = half_float::half_cast<half>(azimuth) + (azimuth_diff * half_float::half_cast<half>(vls_128_laser_azimuth_cache[firing_order]));
            azimuth_corrected = ((uint16_t) round((float)azimuth_corrected_f)) % 36000;
            ...
            if ((azimuth_corrected >= config_.min_angle
              ...
              // convert polar coordinates to Euclidean XYZ
              cos_vert_angle = half_float::half_cast<half>(corrections.cos_vert_correction);
              sin_vert_angle = half_float::half_cast<half>(corrections.sin_vert_correction);
              cos_rot_correction = half_float::half_cast<half>(corrections.cos_rot_correction);
              sin_rot_correction = half_float::half_cast<half>(corrections.sin_rot_correction);

              // cos(a-b) = cos(a)*cos(b) + sin(a)*sin(b)
              // sin(a-b) = sin(a)*cos(b) - cos(a)*sin(b)
              cos_rot_angle =
                half_float::half_cast<half>(cos_rot_table_[azimuth_corrected]) *
                half_float::half_cast<half>(cos_rot_correction) +
                half_float::half_cast<half>(sin_rot_table_[azimuth_corrected]) *
                half_float::half_cast<half>(sin_rot_correction);
              sin_rot_angle =
                half_float::half_cast<half>(sin_rot_table_[azimuth_corrected]) *
                half_float::half_cast<half>(cos_rot_correction) -
                half_float::half_cast<half>(cos_rot_table_[azimuth_corrected]) *
              ...
              y_coord = -(xy_distance * sin_rot_angle); // velodyne x
              z_coord = distance * sin_vert_angle;      // velodyne z
              intensity = half_float::half_cast<half>(current_block.data[k + 2]);
              ...
              if (is_invalid_distance) {
                data.addPoint((float)x_coord, (float)y_coord, (float)z_coord, corrections.laser_ring, azimuth_corrected, 0, (float)intensity, time_stamp);
              }
              else {
                data.addPoint((float)x_coord, (float)y_coord, (float)z_coord, corrections.laser_ring, azimuth_corrected, (float)distance, (float)intensity, time_stamp);
              }
            }
          }
        }
  ...
  /** @brief convert raw VLS128 packet to point cloud with fixed-point(16.16) precision
   *
   *  @param pkt raw packet to unpack
   *  @param pc shared pointer to point cloud (points are appended)
   */
  void RawData::unpack_vls128_fixed_16_16_precision(const velodyne_msgs::VelodynePacket &pkt, DataContainerBase &data) {
    fix16_t last_azimuth_diff = fix16_from_int(0);
    fix16_t azimuth_diff, azimuth_corrected_f;
    uint16_t azimuth, azimuth_next, azimuth_corrected;
    fix16_t x_coord, y_coord, z_coord;
    fix16_t distance, intensity;
    const raw_packet_t *raw = (const raw_packet_t *) &pkt.data[0];
    union two_bytes tmp;

    // float time_diff_start_to_this_packet = (pkt.stamp - scan_start_time).toSec();

    fix16_t cos_vert_angle, sin_vert_angle, cos_rot_correction, sin_rot_correction;
    fix16_t cos_rot_angle, sin_rot_angle;
    fix16_t xy_distance;
    ...
    for (int block = 0; block < BLOCKS_PER_PACKET - (4* dual_return); block++) {
      ...
      if (block < (BLOCKS_PER_PACKET - (1+dual_return))) {
        ...
        // Finds the difference between two sucessive blocks
        azimuth_diff = fix16_from_float((float)((36000 + azimuth_next - azimuth) % 36000) * 0.1f);
        ...
        // same as the last to the second to last.
        // Assumes RPM doesn't change much between blocks
        azimuth_diff = (block == BLOCKS_PER_PACKET - (4*dual_return)-1) ? fix16_from_int(0) : last_azimuth_diff;
      }
      ...
      if ((config_.min_angle < config_.max_angle && azimuth >= config_.min_angle && azimuth <= config_.max_angle) || (config_.min_angle > config_.max_angle)) {
        for (int j = 0, k = 0; j < SCANS_PER_BLOCK; j++, k += RAW_SCAN_SIZE) {
          ...
          // if (data.pointInRange(distance)) {
          {
            ...
            distance = fix16_mul(fix16_from_int(tmp.uint), fix16_from_float(VLP128_DISTANCE_RESOLUTION));
            bool is_invalid_distance = false;
            if(distance == fix16_from_float(0.0)) {
                is_invalid_distance = true;
                distance = fix16_from_float(0.3);
            }
            else {
              distance = fix16_add(distance, fix16_from_float(corrections.dist_correction));
            }

            // correct for the laser rotation as a function of timing during the firings
            azimuth_corrected_f = fix16_add(fix16_from_float((float)azimuth * 0.1f), fix16_mul(azimuth_diff, fix16_from_float(vls_128_laser_azimuth_cache[firing_order])));
            azimuth_corrected = ((uint16_t) round(fix16_to_float(azimuth_corrected_f) * 10.0f)) % 36000;
            ...
            if ((azimuth_corrected >= config_.min_angle
              ...
              // convert polar coordinates to Euclidean XYZ
              cos_vert_angle = fix16_from_float(corrections.cos_vert_correction);
              sin_vert_angle = fix16_from_float(corrections.sin_vert_correction);
              cos_rot_correction = fix16_from_float(corrections.cos_rot_correction);
              sin_rot_correction = fix16_from_float(corrections.sin_rot_correction);

              // cos(a-b) = cos(a)*cos(b) + sin(a)*sin(b)
              // sin(a-b) = sin(a)*cos(b) - cos(a)*sin(b)
              cos_rot_angle =
                fix16_add(
                  fix16_mul(fix16_from_float(cos_rot_table_[azimuth_corrected]), cos_rot_correction),
                  fix16_mul(fix16_from_float(sin_rot_table_[azimuth_corrected]), sin_rot_correction));
              sin_rot_angle =
                fix16_sub(
                  fix16_mul(fix16_from_float(sin_rot_table_[azimuth_corrected]), cos_rot_correction),
                  fix16_mul(fix16_from_float(cos_rot_table_[azimuth_corrected]), sin_rot_correction));

              // Compute the distance in the xy plane (w/o accounting for rotation)
              xy_distance = fix16_mul(distance, cos_vert_angle);
              ...
              //pc.points.push_back(point);
              //++pc.width;
              x_coord = fix16_mul(xy_distance, cos_rot_angle);    // velodyne y
              y_coord = fix16_mul(fix16_from_int(-1), fix16_mul(xy_distance, sin_rot_angle)); // velodyne x
              z_coord = fix16_mul(distance, sin_vert_angle);      // velodyne z
              intensity = fix16_from_int(current_block.data[k + 2]);
              ...
              if (is_invalid_distance) {
                data.addPoint(fix16_to_float(x_coord), fix16_to_float(y_coord), fix16_to_float(z_coord),
                  corrections.laser_ring, azimuth_corrected, 0, fix16_to_float(intensity), time_stamp);
              }
              else {
                data.addPoint(fix16_to_float(x_coord), fix16_to_float(y_coord), fix16_to_float(z_coord),
                  corrections.laser_ring, azimuth_corrected, fix16_to_float(distance), fix16_to_float(intensity), time_stamp);
              }
            }
          }
        }
