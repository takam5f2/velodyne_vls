...
/** @brief Constructor. */
    ...
    base_link_frame_("base_link")
  {
    ...
    num_points_threshold_ = this->declare_parameter(
      "num_points_threshold", 300,
      num_points_threshold_desc);
    ...
    invalid_intensity_double = this->declare_parameter < std::vector <
      double >> ("invalid_intensity");
    ...
    const std::vector < rclcpp::Parameter > & p)
  {
    ...
    std::vector < double > invalid_intensity_double;
    auto it = std::find_if(
      p.cbegin(), p.cend(), [] (const rclcpp::Parameter & parameter) {
...
/** @brief Callback for raw scan messages. */
  void Convert::processScan(const velodyne_msgs::msg::VelodyneScan::SharedPtr scanMsg)
  {
    ...
    pcl::PointCloud < velodyne_pointcloud::PointXYZIRADT >
    ::Ptr invalid_near_points_filtered_xyziradt(
      new pcl::PointCloud < velodyne_pointcloud::PointXYZIRADT >);
    if (
      velodyne_points_invalid_near_pub_->get_subscription_count() > 0 ||
      velodyne_points_combined_ex_pub_->get_subscription_count() > 0)
    {
      ...
      const auto sorted_invalid_points_xyziradt =
        sortZeroIndex(scan_points_xyziradt.pc, num_lasers);
    ...
    if (velodyne_points_combined_ex_pub_->get_subscription_count() > 0) {
      combined_points_xyziradt->points.reserve(
        valid_points_xyziradt->points.size() +
        invalid_near_points_filtered_xyziradt->points.size());
    ...
    const std_msgs::msg::Header & header)
  {
    ...
    const std::array < geometry_msgs::msg::Point, 4 > pos = {
      generatePoint(0.0, 0.0, -0.0285), generatePoint(0.0, 0.0, 0.0),
      generatePoint(0.0, 0.0, 0.0255),
