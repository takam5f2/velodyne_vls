    ...
    const tf2::Transform & tf2_base_link_to_sensor)
  {
    ...
    if (input_pointcloud->points.empty() || twist_queue.empty()) {
      ...
      RCLCPP_WARN_STREAM_THROTTLE(
        rclcpp::get_logger(
          "velodyne_interpolate"), ros_clock, 10000 /* ms */,
        "input_pointcloud->points or twist_queue is empty.");
    ...
    auto twist_it = std::lower_bound(
      ...
      [] (const geometry_msgs::msg::TwistStamped & x,
      rclcpp::Time t) {return rclcpp::Time(x.header.stamp) < t;});
    ...
    for (const auto & p : input_pointcloud->points) {
      for (;
        ...
        p.time_stamp > rclcpp::Time(twist_it->header.stamp).seconds());
        ++twist_it)
      {
